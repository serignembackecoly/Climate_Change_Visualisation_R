# Script to download data from the closest NOAA Stations
...
my_lat <- 12.379389984081858 * 2 * pi / 360
my_lon <- -1.5049498019903216 * 2 * pi / 360
...
local_weather <- read_csv(station_daily,
  ...
  mutate(date = ymd(date), TMAX = TMAX / 10, TMIN = TMIN / 10, PRCP = PRCP /10) %>% 
  rename_all(tolower)

# Checking atypical values
local_weather %>% 
